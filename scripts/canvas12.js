...
function drawColumn() {
	...
	if (families.length < 4) {
		...
		var temp = 6;
		for (var i = 0; i<families.length; i++) {
			if (families[i].member > temp) temp = families[i].member;
		}
		var diff = temp - 6;
		if (diff != 0) {
			if (diff < 11) {
	...
	ptx.closePath();
	var grd1 = ptx.createLinearGradient(x2, y2, x2+4*w2, y2+0.5*h2);
	grd1.addColorStop(0,"#00FF6B");
	grd1.addColorStop(1,"#007832");
	ptx.strokeStyle=grd1;
	ptx.lineWidth=2;
	ptx.stroke();
	...
	ptx.closePath();
	var grd = ptx.createLinearGradient(x1, y1, x1+3*w1, y1+0.5*h1);
	grd.addColorStop(0,"#00FF6B");
	grd.addColorStop(1,"#007832");
	ptx.fillStyle=grd;
	ptx.fill();
		...
		for (var i = 0; i<families.length; i++) {
			if(families[i].member != 0) {
				for (var j = 0; j<families[i].member; j++) {
					var xmetrics = ctx.measureText(families[i].children[j]);
					if (xmetrics.width > (w2-45)) {
						setWrapls(ctx, families[i].children[j],  w2-45, ls);
					}
				}	
			}
		}
	...
	for (var i = 0; i<families.length; i++) {
		if(families[i].member != 0) {
			for (var j = 0; j<families[i].member; j++) {
				...
				var xmetrics = ctx.measureText(families[i].children[j]);
				if (xmetrics.width > (w2-45)) {
					wrap1(ctx, families[i].children[j], xc, yc, w2-45, 13);
				}
				else {
